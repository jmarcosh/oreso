...
def assign_box_number(sp, po, customer, config, log_id):
    ...
    rfid_series_df.loc[free_rfid_rows[:box + 1], C.LOG_ID] = log_id
...
def create_and_save_delivery_note(sp, po_style, customer, delivery_date, config, files_save_path):
    ...
    for po_num in po_nums_lst:
        ...
        subtotal = round(dn['SUBTOTAL'].sum(), 2)
        discount = round(subtotal * dn_discounts, 2)
        subtotal_2 = subtotal - discount
        vat = round(subtotal_2 * .16, 2)
...
def create_po_summary_by_style(po, config):
    ...
    po_gb = po_gb.sort_values(
        by=[C.SECTION, C.RD, C.WAREHOUSE_CODE],
