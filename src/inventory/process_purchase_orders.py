...
def create_and_save_delivery_note(po_style, delivery_date, config, po_num, section, files_save_path):
    ...
    delivery_note = pd.concat([pd.DataFrame(dn_structure), blank_row, dn.T.reset_index().T, dn_totals],
                              ignore_index=True)
    dn_file_path = f"{files_save_path}/Nota_Remision_{delivery_num}.xlsx"
    invoc.save_delivery_note_excel(delivery_note, dn_file_path)

def create_and_save_asn_file(po, config, po_num, files_save_path):
